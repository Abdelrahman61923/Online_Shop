<?php
   ...
   if(isset($_POST['add_to_cart'])){
      ...
      $select_cart = mysqli_query($conn, "SELECT * FROM `addcard` WHERE name = '$product_name' AND user_id = '$user_id'") or die('query failed');

      if(mysqli_num_rows($select_cart) > 0){
         $message[] = 'المنتج أضيف بالفعل إلى عربة التسوق!';
      }else{
         mysqli_query($conn, "INSERT INTO `addcard` (user_id, name, price, image, quantity) VALUES('$user_id', '$product_name', '$product_price', '$product_image', '$product_quantity')") or die('query failed');
   ...
   if(isset($_POST['update_cart'])){
      ...
      mysqli_query($conn, "UPDATE `addcard` SET quantity = '$update_quantity' WHERE id = '$update_id'") or die('query failed');
      $message[] = 'تم تحديث كمية سلة التسوق بنجاح!';
   }

   if(isset($_GET['remove'])){
      $remove_id = $_GET['remove'];
      mysqli_query($conn, "DELETE FROM `addcard` WHERE id = '$remove_id'") or die('query failed');
      header('location:index.php');
   }

   if(isset($_GET['delete_all'])){
      mysqli_query($conn, "DELETE FROM `addcard` WHERE user_id = '$user_id'") or die('query failed');
      header('location:index.php');
   }
...
<body>
   ...
   <div class="container">
      <div class="user-profile">
         ...
         ?>
         <p>المستخدم الحالي : <span><?php echo $row['name']; ?></span> </p>
         <div>
      ...
      <div class="products">
         ...
         <div class="box-container">
            <?php
               $result = mysqli_query($conn, "SELECT * FROM prodect");      
            ...
            <form method="post" class="box" action="">
               ...
               <input type="submit" value="add to cart" name="add_to_cart" class="btn btn-warning">
            </form>
            <?php  endwhile; ?>
         </div>
      </div>
      <div class="shopping-cart">
         ...
         <table>
            ...
            <tbody>
               <?php
                  $cart_query = mysqli_query($conn, "SELECT * FROM `addcard` WHERE user_id = '$user_id'") or die('query failed');
                  $grand_total = 0;
